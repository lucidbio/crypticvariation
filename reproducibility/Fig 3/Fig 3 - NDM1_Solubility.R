...
y2 = list(title="Solubility (%))", range = c(0, 100), titlefont=font, overlaying = "y", side="right")
...
p = plot_ly(data=data) %>%
  ...
  add_trace(x= ~Order,
            ...
            yaxis = "y2",
            name = 'Solubility') %>%
  layout(xaxis=x, yaxis=y1, yaxis2=y2, legend=list(x=100, y=-0.2))

p

plot_json = plotly_json(p, jsonedit = FALSE)
write(plot_json, paste(basename, '_Solubility', '.json', sep=""))
